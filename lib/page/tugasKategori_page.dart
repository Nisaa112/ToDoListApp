...
class TugaskategoriPage extends StatefulWidget {
  const TugaskategoriPage({super.key});

  @override
  State<TugaskategoriPage> createState() => _TugaskategoriPageState();
}

class _TugaskategoriPageState extends State<TugaskategoriPage> {
  @override
  Widget build(BuildContext context) {
    ...
    final int kategoriId = ModalRoute.of(context)!.settings.arguments as int;

    // ✅ Filter tugas berdasarkan kategoriId
    final List<TugasModel> filteredTugas = tugasVM.tugasList
        .where((tugas) => tugas.categoryId == kategoriId)
        .toList();

    // ✅ Ambil nama kategori
    final namaKategori = kategoriVM.kategoriList
        .firstWhere((k) => k.id == kategoriId, orElse: () => kategoriVM.kategoriList.first)
        .name;
